...
function main() {
  ...
  if (args.selfTest) {
    ...
    const report = probeRuntime(fixture.artifact, {
      out: fixture.out,
      allowInvalid: true,
  ...
  if (!args.artifact) {
    console.error("usage: node scripts/lab-runtime-probe.mjs --artifact <capture.json> [--out report.json]");
    console.error("       node scripts/lab-runtime-probe.mjs --self-test [--out report.json]");
    process.exit(2);
  }

  const report = probeRuntime(args.artifact, args);
  console.log(`${report.status.toUpperCase()} ${args.out ?? ""}`.trim());
  if (report.status !== "pass") process.exit(1);
}

export function probeRuntime(artifactPath, options = {}) {
  const record = readCaptureArtifact(artifactPath, options);
  const runtimeReport = measureRuntime(record, options);
  const report = {
    ...runtimeReport,
...
function writeSelfTestArtifact(outPath) {
  const dir = join(repoRoot, "artifacts", "lab-self-test", "runtime-probe");
  mkdirSync(dir, { recursive: true });
  const pngPath = join(dir, "runtime.png");
  writePng(pngPath, 8, 8, makePixels(8, 8));
  const maskPath = join(repoRoot, "fixtures", "masks", "glass_core_mask_pack_v1.json");
  const artifact = join(dir, "runtime.capture.json");
  writeFileSync(artifact, `${JSON.stringify(makeArtifact(pngPath, maskPath), null, 2)}\n`);
  return {
    artifact,
    out: outPath ? resolve(outPath) : join(dir, "g5-runtime.report.json")
  };
}
...
function makeArtifact(pngPath, maskPath) {
  return finalizeCaptureArtifactIntegrity({
    ...
    environment: {
      ...
      reduce_motion: false,
      content_seed: "g5-runtime-self-test",
    ...
    perf: {
      measurement_source: "self_test_fixed_runtime",
      full_frame_ms_p95: 14.2,
      frame_interval_ms_p95: 16.67,
      dropped_frames: 0,
      sustained_degradation_pct: 0.8,
      memory_mb_p95: 92
    },
    energy: {
